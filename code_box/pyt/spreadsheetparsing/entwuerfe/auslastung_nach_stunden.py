...
def check_cmdline_params():
    if len(sys.argv) != 3:
        ...
        if os.path.isdir(sys.argv[1]):
            print(sys.argv[1] + " is a directory. Enable directory mode?")
            ...
            if not ask.lower() == 'y':
                ...
                return sourcefile_IB, targetfile, parsemode
        else:
...
def plot_chart(dictionary):
    dframe = pandas.DataFrame.from_dict(dictionary)
    dframe2 = dframe.transpose()
    dframe2["servicelevel"] = dframe2["servicelevel"]*100
    df3=dframe2[["servicelevel","angekommen","verbunden","verloren"]].plot(title=str(tag))
    df3.set_xlabel("Uhrzeit")
    df3.set_ylabel("Calls / SLA-%")
    plt.show()

def collect_files(filesdir):
    agentsfiles = list()
    for i in os.listdir(filesdir):
        if i.startswith("1458_daily_2"):
...
if mode == "dir":
    files_in_dir=collect_files(source)
    for dati in files_in_dir:
        datei=os.path.join(source,dati)
        tag,kw,stunden = generate_single_day(datei)
        print kw
        print stunden
